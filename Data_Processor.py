...
class Data_Processor:
    ...
    def data_Analysis(self, 
        ...
        if (
            ...
            smoothed_recording = self.smooth_Sound(recording, averaging_window_in_number_of_samples)
    ...
    @classmethod
    def write_Experiment_Data_to_File(self, 
        frequencies, 
        graph_lines, 
        smoothed_recording, 
        time_data, 
    ...
    @classmethod
    def get_Reverberation_Time(self, 
        smooth_recording,           
        starting_intensity,         
        stop_playing_frequency_time 
        ):
    ...
    def get_Starting_intensity(self, smoothed_recording, start_frequency_time, stop_frequency_time):
        ...
        sampling_rate = 44100 # moet ook naar settings file

        starting_intensity_value = int((stop_frequency_time - 1) * sampling_rate)
    ...
    def get_Timestamps_For_Each_Frequency_Test(self, timestamps, frequencies):
        ...
        for frequency in frequencies:
            ...
            for timestamp in timestamps:
                if not (got_start_time or got_stop_time):
    ...
    @classmethod
    def smooth_Sound(self, 
        recording, 
        averaging_window_in_number_of_samples
        ):
        ...
        for _ in recording:
            if index < averaging_window_in_number_of_samples:
                list_of_values = recording[0:(averaging_window_in_number_of_samples + 1)]
                
                smoothed_recording.append(sum(list_of_values) / len(list_of_values))
            
            elif sample_count - index < averaging_window_in_number_of_samples:
                list_of_values = recording[averaging_window_in_number_of_samples:-1]
                
                smoothed_recording.append(sum(list_of_values)/len(list_of_values))
            
            else:
                lower_limit = int(round((index - (averaging_window_in_number_of_samples / 2)), 0))
                upper_limit = int(round((index + (averaging_window_in_number_of_samples / 2)), 0))
